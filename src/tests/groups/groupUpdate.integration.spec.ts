  ...
  describe("GROUP UPDATE USECASES", () => {
    ...
    it("update a created group nike", async () => {
      ...
      const res = await request(app)
        .patch(`/api/groups/${resCreate.body.id}`)
        .send(updateData)
        .expect(200);
      foundNikeGroup = await findOneByQuery("groups", { name: "nike" });
    ...
    it("rename group", async () => {
      ...
      const res = await request(app)
        .patch(`/api/groups/${resCreate.body.id}/rename`)
        .send(updateData)
        .expect(200);
    ...
    it("cannot rename root group", async () => {
      ...
      const updateData = { name: "adidas" };
      const res = await request(app)
        .patch(`/api/groups/${esRes.body.id}/rename`)
        .send(updateData)
        .expect(400);
      expect(res.body.message).toEqual(
        expect.stringContaining(`cannot change root name of`)
      );
    });
    ...
    it("cannot rename group with exists child name in parent ", async () => {
      ...
      const resCreateNike = await request(app)
        .post(`/api/groups`)
        .send(nikeGroup)
        .expect(200);
      nikeGroup = {
      ...
      const resCreateAdidas = await request(app)
        .post(`/api/groups`)
        .send(nikeGroup)
        .expect(200);
      const updateData = { name: "nike" };
      const res = await request(app)
        .patch(`/api/groups/${resCreateAdidas.body.id}/rename`)
        .send(updateData)
        .expect(400);
      expect(res.body.message).toEqual(
        expect.stringContaining(`cannot have two groups with the name`)
      );
    });
  });
};
