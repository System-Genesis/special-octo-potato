...
export const testMoveGroup = () => {
    describe('GROUP MOVE USECASES', () => {
        ...
        const sfGroup = { name: "sf_name", source: 'sf_name'}
        ...
        it('move group to be under another group', async () => {
            ...
            expect(foundAdidasGroup).toMatchObject({name: 'adidas', source: 'es_name', isLeaf: true, directGroup: Types.ObjectId(nikeId)})
        });
        ...
        it('cannot move group to another source', async () => {
            const esRes = await request(app).post(`/api/groups`).send(esGroup).expect(200)
            const esSubGroup = { name: "nike", source: 'es_name', directGroup: esRes.body.id}
            ...
            const sfSubGroup = { name: "sfSub", source: 'sf_name', directGroup: sfRes.body.id}
        ...
        it('cannot move group to another group with already exist child name', async () => {
            ...
            const nikeGroup = { name: "nike", source: 'es_name', directGroup: esRes.body.id}
            const nikeRes = await request(app).post(`/api/groups`).send(nikeGroup).expect(200)
            const nikeGroup2 = { name: "nike", source: 'es_name', directGroup: nikeRes.body.id}
        ...
        it('cannot move group to another group because it creates a cycle', async () => {
            const esRes = await request(app).post(`/api/groups`).send(esGroup).expect(200)
            const esSubGroup = { name: "esSub", source: 'es_name', directGroup: esRes.body.id}
            const esSubRes = await request(app).post(`/api/groups`).send(esSubGroup).expect(200)
            const esSubId = esSubRes.body.id;
            const esSub2Group = { name: "esSub2", source: 'es_name', directGroup: esSubRes.body.id}
