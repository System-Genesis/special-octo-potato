...
const digitalIdentityIdDomains: string[] = config.get(
  "valueObjects.digitalIdentityId.domain.values"
);
    ...
    describe("INVALID CREATIONS", () => {
      ...
      describe("Invalid by personal number error", () => {
        ...
        it("Invalid value object error", async () => {
          const res = await request(app)
            .post(`/api/entities`)
            .send(soldInvalidPersonalNumberValue)
            .expect(400);
          expect(res.body.message).toEqual(
            expect.stringContaining(`invalid personal number: 8517714321321`)
          );
        });
        it("already existed error", async () => {
          await request(app)
            .post(`/api/entities`)
            .send(soldInvalidPersonalNumberExisted)
            .expect(200);
          const res = await request(app)
            .post(`/api/entities`)
            .send(soldInvalidPersonalNumberExisted)
      ...
      describe("Invalid by identity card error", () => {
        ...
        it("Invalid value object error", async () => {
          const res = await request(app)
            .post(`/api/entities`)
            .send(civInvalidIDValue)
            .expect(400);
          expect(res.body.message).toEqual(
            expect.stringContaining(`invalid identity card: 8517714321321`)
          );
        });
        it("already existed error", async () => {
          await request(app)
            .post(`/api/entities`)
            .send(civInvalidIDExisted)
            .expect(200);
          const res = await request(app)
            .post(`/api/entities`)
            .send(civInvalidIDExisted)
          ...
          expect(res.body.message).toEqual(
            expect.stringContaining(
              `identity card: ${civInvalidIDExisted.identityCard} already belogns to another entity`
            )
          );
        });
      });
      ...
      describe("Invalid by goal user id error", () => {
        ...
        it("Invalid value object error", async () => {
          const res = await request(app)
            .post(`/api/entities`)
            .send(civInvalidGoalUserIdValue)
            .expect(400);
          expect(res.body.message).toEqual(
            expect.stringContaining(
              `invalid digital identity unique id: ${civInvalidGoalUserIdValue.goalUserId}`
            )
          );
        });
        it("already existed error", async () => {
          await request(app)
            .post(`/api/entities`)
            .send(civInvalidGoalUserIdExisted)
            .expect(200);
          const res = await request(app)
            .post(`/api/entities`)
            .send(civInvalidGoalUserIdExisted)
          ...
          expect(res.body.message).toEqual(
            expect.stringContaining(
              `GoalUser Id: ${civInvalidGoalUserIdExisted.goalUserId} already belogns to another entity`
            )
          );
        });
      });
      ...
      describe("Invalid by external user error", () => {
        ...
        it("Missing organization value error", async () => {
          const res = await request(app)
            .post(`/api/entities`)
            .send(externalEntityNoOrg)
            .expect(400);
          expect(res.body.message).toEqual(
            expect.stringContaining(
              `Employee creation must supply organization`
            )
          );
        });
        it("Invalid organization value error", async () => {
          const res = await request(app)
            .post(`/api/entities`)
            .send(externalEntityInvalidOrg)
            .expect(400);
          expect(res.body.message).toEqual(
            expect.stringContaining(
              `Invalid organization ${externalEntityInvalidOrg.organization}`
            )
          );
        });
        it("Employee number value error", async () => {
          const res = await request(app)
            .post(`/api/entities`)
            .send(externalEntityInvalidEmpNumber)
            .expect(400);
          expect(res.body.message).toEqual(
            expect.stringContaining(
              `invalid employee id: ${externalEntityInvalidEmpNumber.employeeNumber}`
            )
          );
        });
        it("already existed error", async () => {
          await request(app)
            .post(`/api/entities`)
            .send(externalEntityAlreadyExisted)
            .expect(200);
          const res = await request(app)
            .post(`/api/entities`)
            .send(externalEntityAlreadyExisted)
      ...
      it("Invalid by service type error", async () => {
        const res = await request(app)
          .post(`/api/entities`)
          .send(soldInvalidServiceType)
          .expect(400);
        expect(res.body.message).toEqual(
          expect.stringContaining(
            `invalid service Type: ${soldInvalidServiceType.serviceType}`
          )
        );
      });
      ...
      it("Invalid by rank error", async () => {
        const res = await request(app)
          .post(`/api/entities`)
          .send(soldInvalidRank)
          .expect(400);
        expect(res.body.message).toEqual(
          expect.stringContaining(`invalid rank: ${soldInvalidRank.rank}`)
        );
      });
      ...
      it("Invalid by sex error", async () => {
        const res = await request(app)
          ...
          .expect(400);
        expect(res.body.message).toEqual(
          expect.stringContaining(`${soldInvalidSex.sex} is invalid Sex`)
        );
      });
      ...
      it("Invalid by phone error", async () => {
        const res = await request(app)
          .post(`/api/entities`)
          .send(soldInvalidPhone)
          .expect(400);
        expect(res.body.message).toEqual(
          expect.stringContaining(`invalid phone: ${soldInvalidPhone.phone}`)
        );
      });
      ...
      it("Invalid by mobile phone error", async () => {
        const res = await request(app)
          .post(`/api/entities`)
          .send(soldInvalidMobilePhone)
          .expect(400);
        expect(res.body.message).toEqual(
          expect.stringContaining(
            `invalid mobile phone: ${soldInvalidMobilePhone.mobilePhone}`
          )
        );
      });
