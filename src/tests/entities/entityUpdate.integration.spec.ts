    ...
    describe('ENTITY UPDATE USECASES', () => {
        describe('VALID UPDATE USECASES', () => {
            ...
            it('update a VALID goal user entity', async () => {
                ...
                const resUpdate = await request(app).patch(`/api/entities/${res.body.id}`).send(updateData).expect(200);
                foundEntity = await findOneByQuery('entities', {
            ...
            it('shoudlnt update an invalid entityType', async () => {
                ...
                const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
            });
            describe(`valid aka unit update`, () => {
                it(`Primary belongs to new aka source map`, async () => {
                    ...
                    const resConnectSF = await request(app).put(`/api/entities/${entityId}/digitalIdentity/${soufDI.uniqueId}`).send().expect(200);
                    ...
                    const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(200);
                    foundEntity = await findOneByQuery('entities', {
                        _id: Types.ObjectId(entityId),
                    });
                    const afterCreatedAt = foundEntity.createdAt;
                    const afterUpdatedAt = foundEntity.updatedAt;
                    expect(foundEntity.primaryDigitalIdentityId).toEqual(DigitalIdentityId.create(esDI.uniqueId)._unsafeUnwrap().toValue());
                    expect(Date.parse(beforeCreatedAt) === Date.parse(afterCreatedAt)).toBeTruthy();
                    expect(Date.parse(beforeUpdatedAt) !== Date.parse(afterUpdatedAt)).toBeTruthy();
                });
            });
        ...
        describe(`INVALID update cases`, () => {
            describe(`shouldnt update personal number`, () => {
                ...
                it(`invalid value personal number update`, async () => {
                    ...
                    const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                    expect(resUpdate.body.message).toEqual(expect.stringContaining(`invalid personal number: ${updateData.personalNumber}`));
                });
                it(`personal number already exists update`, async () => {
                    const res = await request(app).post(`/api/entities`).send(soldEntity).expect(200);
                    const res2 = await request(app).post(`/api/entities`).send(soldEntity2).expect(200);
                    ...
                    expect(resUpdate.body.message).toEqual(
                        expect.stringContaining(`personal number: ${updateData.personalNumber} already belogns to another entity`),
                    );
                });
            });
            ...
            describe(`shouldnt update identity card`, () => {
                ...
                it(`invalid value identity card update`, async () => {
                    ...
                    const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                    expect(resUpdate.body.message).toEqual(expect.stringContaining(`invalid identity card: ${updateData.identityCard}`));
                });
                it(`identity card already exists update`, async () => {
                    ...
                    const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                    expect(resUpdate.body.message).toEqual(
                        expect.stringContaining(`identity card: ${updateData.identityCard} already belogns to another entity`),
                    );
                });
            });
            ...
            describe(`shouldnt update goal user id`, () => {
                ...
                it(`invalid value goal user id update`, async () => {
                    ...
                    const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                    expect(resUpdate.body.message).toEqual(expect.stringContaining(`invalid digital identity unique id: ${updateData.goalUserId}`));
                });
                it(`invalid goal user id already exists update`, async () => {
                    const res = await request(app).post(`/api/entities`).send(goalUserEntity).expect(200);
                    const res2 = await request(app).post(`/api/entities`).send(goalUserEntity2).expect(200);
                    ...
                    const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                    expect(resUpdate.body.message).toEqual(
                        expect.stringContaining(`GoalUser Id: ${updateData.goalUserId} already belogns to another entity`),
                    );
                });
            });
            ...
            it(`invalid service type value update`, async () => {
                ...
                const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                expect(resUpdate.body.message).toEqual(expect.stringContaining(`invalid service Type: ${updateData.serviceType}`));
            });
            ...
            it(`invalid rank value update`, async () => {
                ...
                const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                expect(resUpdate.body.message).toEqual(expect.stringContaining(`invalid rank: ${updateData.rank}`));
            });
            ...
            it(`invalid sex value update`, async () => {
                ...
                const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                expect(resUpdate.body.message).toEqual(expect.stringContaining(`${updateData.sex} is invalid Sex`));
            });
            ...
            it(`invalid phone value update`, async () => {
                ...
                const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                expect(resUpdate.body.message).toEqual(expect.stringContaining(`invalid phone: ${updateData.phone}`));
            });
            ...
            it(`invalid mobile phone value update`, async () => {
                ...
                const resUpdate = await request(app).patch(`/api/entities/${entityId}`).send(updateData).expect(400);
                expect(resUpdate.body.message).toEqual(expect.stringContaining(`invalid mobile phone: ${updateData.mobilePhone}`));
            });
        });
    });
};
