...
const userDomains: string[] = config.get('valueObjects.digitalIdentityId.domain.values');
    ...
    describe('ENTITY CONNECT DI USECASES', () => {
        ...
        describe(`invalid connections uses`, () => {
            ...
            it(`Should return uid value error`, async () => {
                ...
                const resConnectES = await request(app).put(`/api/entities/${entityId}/digitalIdentity/${invalidUID}`).send().expect(400);
                expect(resConnectES.body.message).toEqual(expect.stringContaining(`invalid digital identity unique id: ${invalidUID}`));
            });
            it(`Should return entity not found error`, async () => {
                let fakeEntityId = `623b01b9bfc3cbea7b085986`;
                let uid = `uniqueId@${esDomain}`;
                const resConnectES = await request(app).put(`/api/entities/${fakeEntityId}/digitalIdentity/${uid}`).send().expect(404);
                expect(resConnectES.body.message).toEqual(expect.stringContaining(`resource entity: ${fakeEntityId} does not exist`));
            });
            ...
            it(`Should return di not found error`, async () => {
                ...
                const entityId = resEntityCreate.body.id;
                let uid = `uniqueId@${esDomain}`;
                const resConnectES = await request(app).put(`/api/entities/${entityId}/digitalIdentity/${uid}`).send().expect(400);
                expect(resConnectES.body.message).toEqual(expect.stringContaining(`resource digital identity: ${uid} does not exist`));
            });
            ...
            it(`Should return must have upn in order to be connected error`, async () => {
                ...
                expect(resEntityCreate.body.id).toBeTruthy();
                const entityId = resEntityCreate.body.id;
                const resDICreate = await request(app).post(`/api/digitalIdentities`).send(adsDI).expect(200);
                const resConnectES = await request(app).put(`/api/entities/${entityId}/digitalIdentity/${adsDI.uniqueId}`).send().expect(400);

                expect(resConnectES.body.message).toEqual(expect.stringContaining('digital identity must have upn in order to be connected'));
            });
            ...
            it(`Should return logic error(already connected to entity)`, async () => {
                ...
                expect(resConnectES2.body.message).toEqual(
                    expect.stringContaining(`digital identity ${adsDI.uniqueId.toLowerCase()} is already connected to entity`),
                );
            });
        });
        ...
        describe(`invalid disconnect errors`, () => {
            ...
            it(`uid value error`, async () => {
                const res = await request(app).delete(`/api/entities/${fakeEntityId}/digitalIdentity/${invalidUID}`).expect(400);

                expect(res.body.message).toEqual(expect.stringContaining(`invalid digital identity unique id: ${invalidUID}`));
            });

            it(`Should return entity not found error`, async () => {
                const res = await request(app).delete(`/api/entities/${fakeEntityId}/digitalIdentity/${esDI.uniqueId}`).expect(404);
                expect(res.body.message).toEqual(expect.stringContaining(`resource entity: ${fakeEntityId} does not exist`));
            });
            ...
            it(`Should return digital identity not found error`, async () => {
                ...
                const delRes = await request(app).delete(`/api/entities/${entityId}/digitalIdentity/${esDI.uniqueId}`).send(esDI).expect(400);
                expect(delRes.body.message).toEqual(expect.stringContaining(`resource digital identity: ${esDI.uniqueId} does not exist`));
            });
            ...
            it(`Should return entity is not connected error`, async () => {
                ...
                expect(res.body.id).toBeTruthy();
                let entityId = res.body.id;
                await request(app).post(`/api/digitalIdentities`).send(esDI).expect(200);
                const resConnect = await request(app).put(`/api/entities/${entityId}/digitalIdentity/${esDI.uniqueId}`).expect(200);
                let foundDI = await findOneByQuery('digitalidentities', {
                    uniqueId: `uniqueid@${esDomain}`,
                });
                expect(foundDI).toEqual(
                ...
                await request(app).post(`/api/digitalIdentities`).send(adsDI).expect(200);
                const resEntity2 = await request(app).post(`/api/entities`).send(sold2Entity).expect(200);
                ...
                const resConnectWrongDI = await request(app).delete(`/api/entities/${entityId}/digitalIdentity/${adsDI.uniqueId}`).expect(400);
                expect(resConnectWrongDI.body.message).toEqual(
                    expect.stringContaining(`entity: ${entityId} is not connected to digital Identity: ${adsDI.uniqueId}`),
                );
            });
        });
    });
