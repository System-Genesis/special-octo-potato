...
import {
  castToDigitalIdentityType,
  DigitalIdentity,
} from "../domain/DigitalIdentity";
...
export class DigitalIdentityService {
  ...
  async createDigitalIdentity(
    ...
    >
  > {
    const uid = DigitalIdentityId.create(createDTO.uniqueId).mapErr(
    ...
    const type = castToDigitalIdentityType(createDTO.type).mapErr(
      AppError.ValueValidationError.create
    );
    if (type.isErr()) {
    ...
    if (has(createDTO, "mail")) {
      mail = Mail.create(createDTO.mail).mapErr(
        AppError.ValueValidationError.create
      );
      if (mail.isErr()) {
    ...
    return (await this.diRepository.create(digitalIdentity.value)).mapErr(
  ...
  async updateDigitalIdentity(
    ...
    >
  > {
    const uid = DigitalIdentityId.create(updateDTO.uniqueId).mapErr(
      AppError.ValueValidationError.create
    );
    if (uid.isErr()) {
      return err(uid.error);
    ...
    if (!digitalIdentity) {
      return err(
        AppError.ResourceNotFound.create(updateDTO.uniqueId, "digital identity")
      );
    }
    if (has(updateDTO, "mail")) {
      const mail = Mail.create(updateDTO.mail).mapErr(
        AppError.ValueValidationError.create
      );
      if (mail.isErr()) {
        return err(mail.error);
    ...
    return (await this.diRepository.update(digitalIdentity)).mapErr((err) =>
  ...
  async deleteDigitalIdentity(id: string): Promise<Result<void, BaseError>> {
    ...
    return (await this.diRepository.delete(diId.value)).mapErr((err) =>
      AppError.RetryableConflictError.create(err.message)
    );
  }
