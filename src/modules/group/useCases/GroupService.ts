...
export class GroupService {
  ...
  async createGroup(
    ...
    >
  > {
    const source = Source.create(createDTO.source).mapErr(
    ...
    if (has(createDTO, "directGroup")) {
      ...
      if (!parent) {
        return err(
          AppError.ResourceNotFound.create(createDTO.directGroup, "group")
        );
      }
      ...
      if (group.isErr()) {
        const childGroupId = await this.groupRepository.getByNameAndParentId(
          createDTO.name,
          parentId
        );
        if (childGroupId) {
      ...
      if (rootId) {
        return err(
          AppError.AlreadyExistsError.create("group", { id: rootId.toString() })
        );
      }
      group = ok(
    ...
    if (parent) {
      const saveParentRes = (await this.groupRepository.update(parent)).mapErr(
        (err) => AppError.RetryableConflictError.create(err.message)
      );
      if (saveParentRes.isErr())
        return err(
          AppError.UnexpectedError.create(saveParentRes.error.message)
        );
    }
  ...
  async updateGroup(
    ...
    Result<
      GroupResultDTO,
      AppError.ResourceNotFound | AppError.RetryableConflictError
    >
  > {
    const groupId = GroupId.create(updateGroupDTO.id);
    let group: Group | null = await this.groupRepository.getByGroupId(groupId);
  ...
  > {
    ...
    if (!group) {
      return err(
        AppError.ResourceNotFound.create(moveGroupDTO.groupId, "group")
      );
    }
    ...
    if (!parent) {
      return err(
        AppError.ResourceNotFound.create(moveGroupDTO.parentId, "parent group")
      );
    }
    const parentSource = parent.source.value;
    ...
    return (await this.groupRepository.update(group)).mapErr((err) =>
      AppError.RetryableConflictError.create(err.message)
    );
  }
  ...
  async deleteGroup(id: string): Promise<Result<any, BaseError>> {
    ...
    parent.deleteChild();
    const deleteGroupRes = (await this.groupRepository.delete(groupId)).mapErr(
      (err) => AppError.RetryableConflictError.create(err.message)
    );
