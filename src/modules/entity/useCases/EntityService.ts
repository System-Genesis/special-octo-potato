...
export class EntityService {
  ...
  > {
    ...
    if (has(createEntityDTO, "employeeNumber")) {
      ...
      if (organization.isErr()) {
        return err(
          InvalidOrganizationValue.create(createEntityDTO.organization)
        );
      }
      employeeNumber = EmployeeNumber.create(
        createEntityDTO.employeeNumber
      ).mapErr(AppError.ValueValidationError.create);
      ...
      if (
        ...
        return err(
          EmployeeNumberAlreadyExists.create(
            createEntityDTO.employeeNumber,
            createEntityDTO.organization
          )
        );
      }
    }
  ...
  > {
    ...
    const uidOrError = DigitalIdentityId.create(connectDTO.uniqueId).mapErr(
      AppError.ValueValidationError.create
    );
    if (uidOrError.isErr()) {
  ...
  > {
    ...
    const uidOrError = DigitalIdentityId.create(disconnectDTO.uniqueId).mapErr(
      AppError.ValueValidationError.create
    );
    if (uidOrError.isErr()) {
      return err(uidOrError.error);
  ...
  > {
    ...
    if (personalNumber) {
      const newPersonalNumber = PersonalNumber.create(personalNumber).mapErr(
        AppError.ValueValidationError.create
      );
      ...
      if (!entity.personalNumber?.equals(newPersonalNumber.value)) {
        if (await this.entityRepository.exists(newPersonalNumber.value)) {
          return err(
            PersonalNumberAlreadyExistsError.create(
              newPersonalNumber.value.toString()
            )
          );
        }
        changes.push(
          entity.updateDetails({ personalNumber: newPersonalNumber.value })
        );
      }
    }
    if (identityCard) {
      const newIdentityCard = IdentityCard.create(identityCard).mapErr(
        AppError.ValueValidationError.create
      );
      ...
      if (!entity.identityCard?.equals(newIdentityCard.value)) {
        if (await this.entityRepository.exists(newIdentityCard.value)) {
          return err(
            IdentityCardAlreadyExistsError.create(
              newIdentityCard.value.toString()
            )
          );
        }
        changes.push(
          entity.updateDetails({ identityCard: newIdentityCard.value })
        );
      }
    }
    if (goalUserId) {
      const newGoalUserId = DigitalIdentityId.create(goalUserId).mapErr(
        AppError.ValueValidationError.create
      );
      ...
      if (!entity.goalUserId?.equals(newGoalUserId.value)) {
        if (await this.entityRepository.exists(newGoalUserId.value)) {
          return err(
            GoalUserIdAlreadyExistsError.create(newGoalUserId.value.toString())
          );
        }
    ...
    if (entityType) {
      const newEntityType = castToEntityType(entityType).mapErr(
        AppError.ValueValidationError.create
      );
    ...
    if (serviceType) {
      const newServiceType = ServiceType.create(serviceType).mapErr(
        AppError.ValueValidationError.create
      );
      if (newServiceType.isOk()) {
        changes.push(
          entity.updateDetails({ serviceType: newServiceType.value })
        );
      } else {
    ...
    if (rank) {
      const newRank = Rank.create(rank).mapErr(
        AppError.ValueValidationError.create
      );
      if (newRank.isOk()) {
    ...
    if (sex) {
      const newSex = castToSex(sex).mapErr(
        AppError.ValueValidationError.create
      );
      if (newSex.isOk()) {
    ...
    if (mobilePhone) {
      const newMobilePhone = combine(
        toArray(mobilePhone).map(MobilePhone.create)
      )
        .mapErr(AppError.ValueValidationError.create)
        .map(UniqueArray.fromArray);
      if (newMobilePhone.isOk()) {
        changes.push(
          entity.updateDetails({ mobilePhone: newMobilePhone.value })
        );
      } else {
    ...
    if (akaUnit) {
      ...
      const connectedDIs = (
        await this.diRepository.getByEntityId(entityId)
      ).map((di) => di.connectedDigitalIdentity);
