...
export class EntityService {
    ...
    > {
        ...
        if (has(createEntityDTO, 'personalNumber')) {
            personalNumber = PersonalNumber.create(createEntityDTO.personalNumber).mapErr(AppError.ValueValidationError.create);
        ...
        if (has(createEntityDTO, 'phone')) {
            phone = combine(toArray(createEntityDTO.phone).map(Phone.create)).mapErr(AppError.ValueValidationError.create).map(UniqueArray.fromArray);
            if (phone.isErr()) {
        ...
        const entityOrError = Entity.createNew(this.entityRepository.generateEntityId(), {
            ...
            fullClearance: createEntityDTO.fullClearance,
    ...
    async disconnectDigitalIdentity(
        ...
        Result<void, AppError.ValueValidationError | AppError.ResourceNotFound | EntityIsNotConnectedError | AppError.RetryableConflictError>
    > {
    ...
    > {
        ...
        if (phone) {
            const newPhone = combine(toArray(phone).map(Phone.create)).mapErr(AppError.ValueValidationError.create).map(UniqueArray.fromArray);
            if (newPhone.isOk()) {
