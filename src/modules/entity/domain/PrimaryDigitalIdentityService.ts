...
export class PrimaryDigitalIdentityService {
    ...
    static _primarySourceMap: Map<string, Source> = new Map(
        Object.entries(PRIMARY_MAP).map(([key, source]) => [key, wrapResult(Source.create(source as string))]),
    );
    ...
    static havePrimarySource(akaUnit: string | undefined, digitalIdentity: IConnectedDI) {
        return !!akaUnit ? digitalIdentity.source.value === PrimaryDigitalIdentityService._primarySourceMap.get(akaUnit)?.value : false;
    }
