...
const ENTITY_TYPE_VALID_STATE: {
    ...
    GoalUser: {
        ...
        forbidden: ['identityCard', 'employeeNumber', 'rank', 'serviceType', 'sex', 'address', 'dischargeDay', 'birthDate'],
    },
    External: {
...
export type UpdateResult = Result<void, IllegalEntityStateError | CannotChangeEntityTypeError | AppError.CannotUpdateFieldError>;
...
export class Entity extends AggregateRoot {
    ...
    public updateDetails(updateDto: UpdateDto): UpdateResult {
        ...
        if (has(updateDto, 'entityType') && !Entity.isValidEntityTypeTransition(this._state.entityType, updateDto.entityType)) {
    ...
    public updateProfilePicture(update: ProfilePicture): Result<void, IllegalEntityStateError> {
    ...
    private static isValidEntityState(state: EntityState): Result<void, IllegalEntityStateError> {
        ...
        if (state.entityType === entityTypes.GoalUser) {
            if (!!state.goalUserId && !!state.primaryDigitalIdentityId && !state.goalUserId.equals(state.primaryDigitalIdentityId)) {
                return err(IllegalEntityStateError.create(`goalUserId must be the same as primaryDigitalIdentityId`));
            }
        }
        return ok(undefined);
    }

    private static isValidEntityTypeTransition(from: string, to: string): boolean {
        if (from !== to && (from === entityTypes.GoalUser || to === entityTypes.GoalUser)) {
            return false;
        }
        return true;
    }
    ...
    public choosePrimaryDigitalIdentity(connectedDIs: DigitalIdentityRepresent[]) {
        ...
        if (!!currentPrimary && PrimaryDigitalIdentityService.haveStrongSource(currentPrimary)) {
            return;
        }
        ...
        if ((!currentPrimary || !PrimaryDigitalIdentityService.havePrimarySource(this._state.akaUnit, currentPrimary)) && !!primarySourceDI) {
